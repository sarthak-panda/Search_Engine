...
int main()
{
    ...
    for (int i = 1; i <= num_books; i++)
    {
        ...
        while (std::getline(inputFile, tuple, ')') && std::getline(inputFile, sentence))
        {
            ...
            qna_tool.insert_sentence_2(metadata[0], metadata[1], metadata[2], metadata[3], sentence);
        }
    ...
    */
    ...
    Node *head = qna_tool.get_top_k_para_2(question, 45);
    vector<string> paras;
    while (head != nullptr)
    {
        cout << head->book_code << " " << head->page << " " << head->paragraph << " " << head->offset << endl;
